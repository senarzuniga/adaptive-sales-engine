...
export function DataPreviewTables() {
  const { data } = useData();
  const [page, setPage] = useState<Record<string, number>>({ orders: 0, opportunities: 0, products: 0, strategy: 0 });

  const hasAny = data.orders.length > 0 || data.opportunities.length > 0 || data.products.length > 0 || data.strategy.length > 0;
  if (!hasAny) return null;

  const defaultTab = data.orders.length > 0 ? 'orders' : data.opportunities.length > 0 ? 'opportunities' : data.products.length > 0 ? 'products' : 'strategy';
