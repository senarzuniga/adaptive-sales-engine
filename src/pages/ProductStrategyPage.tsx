                            ...
                            {(selectedDraft.costPreset || []).map((line, index) => (
                              <div key={`${selectedDraft.draftId}-${index}`} className="rounded-lg border bg-background p-3 space-y-3">
                                ...
                                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                                  ...
                                  <div><label className="text-xs text-muted-foreground">Structure overhead %</label><Input type="number" step="0.5" aria-label={`Structure overhead ${line.lineItem || index + 1}`} value={line.structurePct ?? 0} onChange={(e) => updateCostPresetLine(selectedDraft.draftId, index, { structurePct: Number(e.target.value || 0) })} disabled={!isEditing} /></div>
