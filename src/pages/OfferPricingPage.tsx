...
const CATEGORIES_ES: Record<string, string> = {
  ...
  subcontracting: 'Subcontrataci√≥n',
...
export default function OfferPricingPage() {
  ...
  const applyIngecartOfferTemplate = () => {
    ...
    toast({
      ...
      description: isEs ? 'Se aplican los valores de garant√≠a, viajes, instalaci√≥n y estructura comercial recomendados.' : 'The recommended cost, travel and installation commercial policy values have been applied.',
    });
  };
  ...
  const runAnalysis = async () => {
    if (totals.total === 0) {
      toast({ title: isEs ? 'Sin datos de costes' : 'No cost data', description: isEs ? 'A√±ade l√≠neas de coste primero' : 'Add cost lines first', variant: 'destructive' });
      return;
    }
    setAnalyzing(true);
    try {
      ...
      toast({ title: isEs ? 'An√°lisis completado' : 'Analysis complete' });
    } catch (e: any) {
    ...
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {isEs ? 'Analiza costes, simula escenarios, eval√∫a riesgos y optimiza precios con IA' : 'Analyze costs, simulate scenarios, evaluate risks and optimize pricing with AI'}
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="analysis">{isEs ? 'An√°lisis' : 'Analysis'}</TabsTrigger>
        ...
        <TabsContent value="builder" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="text-sm font-medium text-foreground">{isEs ? 'T√≠tulo' : 'Title'}</label>
                <Input value={offerTitle} onChange={e => setOfferTitle(e.target.value)} placeholder={isEs ? 'Ej: L√≠nea de ensamblaje' : 'E.g: Assembly line'} />
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                <label className="text-sm font-medium text-foreground">{isEs ? 'Descripci√≥n del proyecto' : 'Project Description'}</label>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">{isEs ? 'Pol√≠tica de costes y plantilla' : 'Cost policy & template'}</CardTitle>
              <CardDescription>{isEs ? 'Aplica la configuraci√≥n base Ingecart para garant√≠as, financiaci√≥n, gesti√≥n comercial, materiales e instalaci√≥n.' : 'Apply the Ingecart default commercial cost policy for warranty, financing, sales management, materials and installation.'}</CardDescription>
            </CardHeader>
            <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="rounded-md border bg-muted/20 p-3">
                <div className="text-xs text-muted-foreground">{isEs ? 'Garant√≠a' : 'Warranty'}</div>
                <div className="font-semibold">{pricingPolicy.warrantyPct}%</div>
              </div>
              <div className="rounded-md border bg-muted/20 p-3">
                <div className="text-xs text-muted-foreground">{isEs ? 'Financiaci√≥n' : 'Finance'}</div>
                <div className="font-semibold">{pricingPolicy.financialPct}%</div>
              </div>
              <div className="rounded-md border bg-muted/20 p-3">
                <div className="text-xs text-muted-foreground">{isEs ? 'Gesti√≥n comercial' : 'Commercial mgmt.'}</div>
          ...
          {data.products.length > 0 && (
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base">{isEs ? 'Cat√°logo de productos y servicios' : 'Product & service catalog'}</CardTitle>
                <CardDescription>{isEs ? 'Selecciona un elemento validado para a√±adirlo a la oferta.' : 'Select a validated catalog item and add it to this offer.'}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="flex flex-col md:flex-row gap-2">
                  ...
                  <Button variant="outline" onClick={addCatalogItem} disabled={!catalogSelection}>
                    ...
                    {isEs ? 'AÒadir del cat·logo' : 'Add from catalog'}
                  </Button>
                </div>
                {selectedCatalogProduct ? (
                  <div className="grid gap-3 md:grid-cols-3 rounded-lg border bg-muted/20 p-3 text-sm">
                    ...
                    <div className="space-y-2">
                      ...
                      <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={catalogIncludeInstallation} onChange={e => setCatalogIncludeInstallation(e.target.checked)} />{isEs ? 'Incluir instalaciÛn' : 'Include installation'}</label>
                      <p className="text-xs text-muted-foreground">{isEs ? 'Al aÒadir el producto se cargar·n sus lÌneas de coste reutilizables en la oferta.' : 'Adding the product loads its reusable cost lines into the offer.'}</p>
                    </div>
                  </div>
                ) : null}
              </CardContent>
              ...
              {expandedItems.has(item.id) && (
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    <div>
                      <label className="text-xs font-medium text-foreground">{isEs ? 'Descripci√≥n' : 'Description'}</label>
                      ...
                      <div key={cat.value} className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => addCostLine(item.id, cat.value)}>
                            <Plus className="h-3 w-3 mr-1" />{isEs ? 'A√±adir' : 'Add'}
                          </Button>
                        </div>
                        {lines.length > 0 && (
                                ...
                                <TableRow>
                                  ...
                                  {cat.value === 'engineering' ? (
                                    <>
                                      ...
                                      <TableHead>{isEs ? '‚Ç¨/h' : '‚Ç¨/h'}</TableHead>
                                    </>
                                  ) : cat.value === 'installation' ? (
                                    <>
                                      <TableHead>{isEs ? 'D√≠as' : 'Days'}</TableHead>
                                      <TableHead>{isEs ? 'Recursos' : 'Resources'}</TableHead>
                                      <TableHead>{isEs ? 'Coste/d√≠a' : 'Cost/day'}</TableHead>
          ...
          <Button variant="outline" className="w-full" onClick={addItem}>
            <Plus className="h-4 w-4 mr-2" />
            {isEs ? 'A√±adir Producto/Servicio' : 'Add Product/Service'}
          </Button>
        ...
        <TabsContent value="analysis" className="space-y-4">
          {!analysis ? (
            <Card><CardContent className="py-12 text-center text-muted-foreground">
              ...
              <p>{isEs ? 'Ejecuta el an√°lisis IA para ver resultados' : 'Run AI analysis to see results'}</p>
            </CardContent></Card>
          ) : (
            <>
              {/* Scenarios */}
                  ...
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      {analysis.profitabilityControl.belowThreshold && (
                        <Badge variant="destructive">{isEs ? '‚ö† BAJO UMBRAL' : '‚ö† BELOW THRESHOLD'}</Badge>
                      )}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {analysis.profitabilityControl.minimumMarginScenario && (
                        <div className="p-3 rounded-lg bg-muted/50">
                          <p className="text-xs text-muted-foreground">{isEs ? 'Margen M√≠nimo Escenario' : 'Minimum Margin Scenario'}</p>
              ...
              {analysis.costAnalysis?.rateValidation && analysis.costAnalysis.rateValidation.length > 0 && (
                <Card>
                  <CardHeader><CardTitle className="flex items-center gap-2"><Settings2 className="h-5 w-5 text-primary" />{isEs ? 'Validaci√≥n de Tasas' : 'Rate Validation'}</CardTitle></CardHeader>
                  <CardContent>
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead>{isEs ? 'Desviaci√≥n' : 'Deviation'}</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                ...
                <Card className="border-destructive">
                  <CardContent className="pt-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-medium text-destructive">{isEs ? 'Categor√≠as de Coste Faltantes' : 'Missing Cost Categories'}</p>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>{isEs ? 'T√≠tulo' : 'Title'}</TableHead>
                        ...
                        <TableCell>
                          <Select value={o.status} onValueChange={(v) => updateOfferStatus(o.id, v)}>
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="negotiation">{isEs ? 'Negociaci√≥n' : 'Negotiation'}</SelectItem>
        ...
        <TabsContent value="summary">
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><TrendingUp className="h-5 w-5 text-primary" />{isEs ? 'Resumen Econ√≥mico' : 'Economic Summary'}</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="p-4 rounded-lg bg-muted/50 text-center">
                  <p className="text-xs text-muted-foreground">{isEs ? 'Margen ‚Ç¨' : 'Margin ‚Ç¨'}</p>
                  <p className="text-xl font-bold text-green-600">{fmt(totals.margin)}</p>
                </div>
                <div className="p-4 rounded-lg bg-muted/50 text-center">
                  <p className="text-xs text-muted-foreground">{isEs ? 'Margen %' : 'Margin %'}</p>
              ...
              <Separator />
              <h3 className="font-semibold text-foreground">{isEs ? 'Desglose por categor√≠a' : 'Breakdown by Category'}</h3>
              <div className="space-y-2">
                {CATEGORIES.map(c => {
