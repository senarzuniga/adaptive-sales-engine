...
def parse_offers(context_map, offer_documents):
    ...
    for row in ws.iter_rows(min_row=3, values_only=True):
        ...
        offers.append(offer)
        order_effective_date = final_decision if re.match(r'\d{4}-\d{2}-\d{2}', final_decision or '') else submitted
        if status == 'won':
            orders.append({'poDate': order_effective_date, 'firstOfferDate': submitted, 'oppNumber': offer['offer_number'], 'region': region, 'country': country, 'customerName': customer_name, 'scope': concept, 'productFamily': concept, 'segment': 'Industrial automation', 'purchasingYear': order_effective_date[:4], 'purchasingQuarter': quarter_from_date(order_effective_date), 'purchasingMonth': datetime.fromisoformat(order_effective_date).strftime('%B') if re.match(r'\d{4}-\d{2}-\d{2}', order_effective_date) else '', 'sellingPrice': value, 'margin': 22 if value >= 100000 else 28, 'kam': clean(row[5]) or 'Ingecart'})
        else:
