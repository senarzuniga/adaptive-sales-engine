...
from domain.erp import ERPSystem
from domain.models import ActionStatus, FeedbackType

erp = ERPSystem()

def page_erp():
    st.title("ERP: Gestión de Proyectos y Operaciones")
    import random
    actions_prev = [erp.create_action(
        title=f"Acción {i+1}",
        description=f"Descripción {i+1}",
        assigned_to=random.choice(["admin","usuario","compras","costes"]),
        entity_type="project",
        entity_id=f"project_{i+1}",
        action_type="task",
        priority=random.randint(1,10),
        context={}
    ) for i in range(10)]
    actions = erp.get_open_actions()
    total = len(actions)
    cerradas = len([a for a in erp.actions if getattr(a, "status", None) == "closed"])
    ...
    st.subheader("🔎 Histórico y buscador global de acciones, proyectos y operaciones (ERP)")
    search_query = st.text_input("Buscar por palabra clave, usuario, estado o tipo", key="erp_search")
    filter_status = st.selectbox("Filtrar por estado", ["Todos", "open", "in_progress", "waiting", "closed"], key="erp_status")
    filter_user = st.text_input("Filtrar por usuario asignado", key="erp_user")
    ...
    st.subheader("🕒 Actividad reciente y accesos rápidos (ERP)")
    recent = sorted(
        actions,
    ...
    for row in recent:
        ...
        if st.button(f"Ver detalle de {getattr(row, 'title', '')}", key=f"ver_detalle_erp_{getattr(row, 'id', '')}"):
    ...
    for action in actions:
        with st.expander(f"{getattr(action, 'title', '')} [{getattr(action, 'status', '')}] - {getattr(action, 'assigned_to', '')}"):
            ...
            if st.button(f"Cerrar acción {getattr(action, 'id', '')}"):
                erp.close_action(getattr(action, 'id', ''))
            ...
            if st.button(f"Enviar feedback {getattr(action, 'id', '')}"):
                erp.add_feedback(getattr(action, 'id', ''), "usuario", FeedbackType(feedback), comments)
                st.success("Feedback registrado y loop gestionado")
    # Crear nueva acción
    st.header("Crear nueva acción")
    if st.button("Crear acción de ejemplo"):
        erp.create_action(
            title="Revisar desviación de costes",
            description="Analizar desviación de costes en proyecto X",
            assigned_to="usuario",
            entity_type="project",
            entity_id="project_1",
            action_type="task",
            priority=8,
            context={"desviacion": 12.5}
        )
        st.success("Acción creada")
    # Resumen ejecutivo automático
    if st.button("Generar resumen ejecutivo semanal"):
        st.info(f"Resumen semanal: {cerradas} acciones cerradas, {abiertas} abiertas. Top usuario: " + (max(set([a.assigned_to for a in erp.actions if getattr(a, 'status', None) == 'closed']), key=[a.assigned_to for a in erp.actions if getattr(a, 'status', None) == 'closed'].count) if cerradas else "N/A"))
    # Accesibilidad
